...
-- mostrar un listado de todos los clientes que Ruben Sila 

select * from cliente where Direccion in 
(select Direccion from cliente where RFCCliente = 'YYYYYYYY' )

--  Listar las agencias con autos pontiac 
select * from autos where Marca = 'pontiac'

select distinct ag.IdAgencia,ag.NombreAgencia,ag.garage from Agencia ag
join autos a on a.IdAgencia = ag.IdAgencia
where a.Marca = 'pontiac' 

-- Mostrar las reservaciones cuyo precio total sean mayores al precio de la reservacion no.10 

select * from Reservacion where precioTotal in ( select precioTotal from Reservacion where precioTotal > 5700)

-- Mostrar todas las marcas de los autos que no maneja la agencia norte 

SELECT distinct marca FROM autos
WHERE Marca NOT IN (select a.marca from autos a
